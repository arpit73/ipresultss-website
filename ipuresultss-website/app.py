...
client = pymongo.MongoClient(
    "mongodb+srv://xxx:xxx@cluster0-gclx4.mongodb.net/test?retryWrites=true&w=majority"
)
db = client.test
app = Flask(__name__)
    ...
    if request.method == "GET":
        ...
        try:
            ...
            return render_template(
                "home.html", err="Valid roll no not found. Try another."
            )
        else:
            df = df[
                (df["batch"] == inp_batch)
                & (df["college"] == inp_college)
                & (df["class"] == inp_class)
            ]
            df = df.fillna(0)
            df["tot0"] = (
            ...
            for i in range(9):
                ...
                df["rank" + str(i)] = df["per" + str(i)].rank(
